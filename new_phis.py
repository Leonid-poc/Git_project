...
class Player(pygame.sprite.Sprite):
    ...
    def send_hp(self):
        if self.god_mode:
            pass
        return FONT.render(f'{self.hp}', False, (255, 0, 0))
...
class Projectale(pygame.sprite.Sprite):
    def __init__(self, person, god_mode=False):
        super().__init__(projectales)
        self.mode = god_mode
        self.pers_pos_x, self.pers_pos_y = person.x, person.y
        if self.mode:
...
while True:
    ...
    # Отрисовка кол-ва хп
    screen.blit(Player.send_hp(), (150, 0))
    # Смена кадра
    pygame.display.flip()
    clock.tick(FPS)
